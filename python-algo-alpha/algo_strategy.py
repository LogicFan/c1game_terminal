...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def init_defense(self, game_state):
        # add filter for edge
        game_state.attempt_spawn(FILTER, [[0, 13], [1, 13], [2, 13], [25, 13], [26, 13], [27, 13]])
        # add destuctor
        game_state.attempt_spawn(DESTRUCTOR, [[4, 11], [23, 11], [9, 6], [18, 6]])
        # add filter for adaptive defense
        for i in range(0, 8):
            game_state.attempt_spawn(FILTER, [[2 + i, 13 - i], [25 - i, 13 - i]])
        for i in range(0, 5):
            game_state.attempt_spawn(FILTER, [[9 + i, 6], [18 - i, 6]])
    ...
    def adaptive_defense(self, game_state):
        ...
        def protect(x, y):
            ...
            for unit in units:
                if unit.unit_type == DESTRUCTOR:
                    game_state.attempt_spawn(FILTER, [x, y + 1])
                    return True
                elif unit.stationary:
                    return True
            return False

        def protect_left(x, y):
            units = game_state.game_map[x, y]
            for unit in units:
                if unit.unit_type == DESTRUCTOR:
                    game_state.attempt_spawn(FILTER, [[x, y + 1], [x - 1, y]])
                    return True
                elif unit.stationary:
                    return True
            return False

        def protect_right(x, y):
            units = game_state.game_map[x, y]
            for unit in units:
                if unit.unit_type == DESTRUCTOR:
                    game_state.attempt_spawn(FILTER, [[x, y + 1], [x + 1, y]])
        ...
        defense_list = []
        # get list of destoryed filter
        for i in range(0, 8):
            if not protect_right(2 + i, 13 - i):
                defense_list.append([2 + i, 13 - i])
            if not protect_left(25 - i, 13 - i):
                defense_list.append([25 - i, 13 - i])
        for i in range(0, 5):
            if not protect(9 + i, 6):
                defense_list.append([9 + i, 6])
            if not protect(18 - i, 6):
                defense_list.append([18 - i, 6])
        
        # caculate can put how many, based on core
        core = game_state.get_resource(game_state.CORES)
        filt_cost = game_state.type_cost(FILTER)
        dest_cost = game_state.type_cost(DESTRUCTOR)

        num = int((core - len(defense_list) * filt_cost) // (dest_cost - filt_cost))
        num = max(0, num)

        dest_list = defense_list[:num]
        filt_list = defense_list[num:]

        if(len(dest_list) != 0):
           game_state.attempt_spawn(DESTRUCTOR, dest_list)
        if(len(filt_list) != 0):
           game_state.attempt_spawn(FILTER, filt_list)
        
    def add_encryptor(self, game_state):
        encr_list = [[10, 5], [17, 5], [11, 5], 
                     [16, 5], [11, 4], [16, 4], 
                     [12, 1], [15, 1], [13, 1], [14, 1]]
        game_state.attempt_spawn(ENCRYPTOR, encr_list)
