...
def preproc_density(game_state, densityV1, densityV2):
    ...
    numpy.clip(quantityV1, a_min=0, a_max=20, out=quantityV1)
    numpy.clip(quantityV2, a_min=0, a_max=20, out=quantityV2)
    quantityV1 = numpy.vectorize(probabilistic_round)(quantityV1)
    quantityV2 = numpy.vectorize(probabilistic_round)(quantityV2)
    ...
    return quantityV1, densityV1, quantityV2, densityV2
...
def totalScore_V2(inS):
    def depreciation(x):
        ...
        return math.tanh(x // 3) * 2.5 - abs(x - 3) * 0.5
    score = 0
    score += inS[IN_S_SELF_HEALTH] * 10
...
def loadSample(path_base):
    ...
    try:
        ...
        if a_inS[-1][IN_S_SELF_HEALTH] > a_inS[-1][IN_S_ENEMY_HEALTH]:
            a_bias = 1
            b_bias = -1
        elif a_inS[-1][IN_S_SELF_HEALTH] < a_inS[-1][IN_S_ENEMY_HEALTH]:
            a_bias = -1
            b_bias = 1
        else:
            a_bias = 0
...
def create():
    ...
    procV = keras.layers.Dense(ARENA_VOL * IN_V_SIZE, activation="relu")(procV)
